...
async function dbQueries(){
    ...
    try{
      //const students = await studentModel.find({name:"Rasmeet"}) []
      //const students = await studentModel.findOne({name:"Kaur"})  null
     ...
     */
    const stds = await studentModel.where("age").gt("10").where("name").equals("Rasmeet").limit(2)
    console.log(stds)

    

    /**
     * Delete one document where name = "Rasmeet"
     */

    const student = await studentModel.deleteOne({name : "Rasmeet"})
    console.log(student)
